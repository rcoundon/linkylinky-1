...
gulp.task('clean-css', function() {
  return gulp.src(buildDest + '/css/**/*', { read: false }).pipe(clean());
});
// Delete our old js files
gulp.task('clean-js', function() {
  return gulp.src(buildDest + '/js/**/*', { read: false }).pipe(clean());
});
...
gulp.task('render', function() {
  gulp.src([buildSrc + '/pages/**/[!_]*.html']).pipe(gulp.dest(buildDest));
});
...
gulp.task('js', function() {
  gulp.src(buildSrc + '/js/**/*.js').pipe(gulp.dest(buildDest + '/js'));
});
  ...
  request(url, function(err, response, body) {
    ...
    if (!err && response.statusCode === 200) {
      ...
      for (var item in formsData) {
        ...
        routes.push(
          '/' + formsData[item].data.code + '  ' + destination + '  302'
        );
      }
...
gulp.task('build', function(callback) {
  runSequence('clean-build', ['get:routes', 'render', 'js', 'scss'], callback);
});
